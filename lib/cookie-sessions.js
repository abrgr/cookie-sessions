...
var exports = module.exports = function(settings){
    ...
    return function(req, res, next){
        ...
        req.getSessionData = function() {
            ...
            return escape(exports.serialize(s.secret, req.session));
        };
        ...
        res.writeHead = function(statusCode){
            ...
            var serializedSessionData = req.getSessionData();

            // the app can request that we skip setting the session cookie
...
exports.readSession = function(key, secret, timeout, req){
    ...
    // prefer cookies but accept x-auth-token header if cookie isn't present
    if ( req.headerSession ) {
        if(req.headers['x-auth-token']){
            return exports.deserialize(secret, timeout, req.headers['x-auth-token']);
        }
    } else if ( req.querySession ) {
        if(req.query.authToken){
            return exports.deserialize(secret, timeout, req.query.authToken);
        }
    } else {
        if(cookies[key]){
